...
public class RenderEntityTrailSecond extends RenderLivingBase<TrailRenderHandler.EntityTrailSecond> {

    public float alpha;
    private static ModelPlayer MODEL = new ModelPlayer(0F, false);
    private static ModelPlayer MODEL_SLIM = new ModelPlayer(0F, true);

    public RenderEntityTrailSecond(RenderManager renderManagerIn) {
        super(renderManagerIn, new ModelPlayer(0F, false), 0F);
    }

    @SuppressWarnings({"unchecked", "rawtypes"})
    @Override
    public void doRender(TrailRenderHandler.EntityTrailSecond entity, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        if (trailType == null || !trailType.getTrailRenderer().preRenderSecondSpeedMirage(entity, trailType, partialTicks))
            return;
    ...
    @Override
    protected void preRenderCallback(TrailRenderHandler.EntityTrailSecond entitylivingbaseIn, float partialTickTime) {
        GlStateManager.color(1F, 1F, 1F, this.alpha);
    }

    @Override
    protected ResourceLocation getEntityTexture(TrailRenderHandler.EntityTrailSecond entity) {
